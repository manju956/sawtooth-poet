...
int main(int argc, char **argv) {
    ...
    test_poetVerifyWaitCert(waitCert.data(), waitCert.length, waitCertSig.data(), poetPublicKey.data());
...
void test_poetCreateSignupData(char *poetPublicKey, size_t poetPubKeyLen,
    ...
    std::string pubKeyHash = "ABCD1234";
    poet_err_t ret = Poet_CreateSignupData(pubKeyHash.c_str(), pubKeyHash.length(),
					   poetPublicKey, poetPubKeyLen,
                                           enclaveQuote, enclaveQuoteLen);
    if(ret != POET_SUCCESS) {
...
void test_poetVerifySignupInfo(char *poetPublicKey, char *enclaveQuote) {
    std::string pubKeyHash = "ABCD1234";
    poet_err_t ret = Poet_VerifySignupInfo(pubKeyHash.c_str(), pubKeyHash.length(),
					   poetPublicKey, enclaveQuote);
    if(ret != POET_SUCCESS){
...
void test_poetVerifyWaitCert(char *serialWaitCert, size_t serialWaitCertLen,
			     char *serialWaitCertSig, char *poetPublicKey) {

    poet_err_t ret = Poet_VerifyWaitCertificate(serialWaitCert,
                                                serialWaitCertLen,
                                                serialWaitCertSig,
                                                poetPublicKey);
...
void test_enclaveCharacteristics() {
    ...
    poet_err_t ret = Poet_GetEnclaveCharacteristics( mrEnclaveBuffer.data(),
                                    ...
                                    basenameBuffer.length, epidGroupBuffer.data(),
