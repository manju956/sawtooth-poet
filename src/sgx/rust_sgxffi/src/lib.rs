...
mod tests {
    ...
    fn initialize_enclave(eid: &mut r_sgx_enclave_id_t) -> String{
        ...
        let ret = ffi::init_enclave(eid, bin_path, spid_str)
    ...
    fn test_init_enclave() {
        let mut eid:r_sgx_enclave_id_t = 
                                r_sgx_enclave_id_t {handle : 0,
    ...
    fn create_signup_info(eid: &mut r_sgx_enclave_id_t, 
                          signup_info: &mut r_sgx_signup_info_t,
                          opk_hash_vec: &str) -> String {
    ...
    fn initialize_wait_certificate(eid: &mut r_sgx_enclave_id_t, 
                                   signup_info: &mut r_sgx_signup_info_t) ->String {
        ...
        let poet_public_key = ffi::create_string_from_char_ptr(
                                signup_info.poet_public_key as *mut c_char);

        let ret = ffi::initialize_wait_cert(eid, &mut duration,
                                            &prev_cert, &prev_wait_cert_sig,
                                            &validator_id, &poet_public_key)
                            .expect("Failed to initialize Wait certificate");
        ret
    }

    fn finalize_wait_certificate(eid: &mut r_sgx_enclave_id_t, 
                wait_cert_info: &mut r_sgx_wait_certificate_t) -> String {
        let prev_cert = "";
        ...
        let ret = ffi::finalize_wait_cert(eid, wait_cert_info, &prev_cert,
    ...
    fn test_create_wait_cert() {
        ...
        let wait_cert_sig = ffi::create_string_from_char_ptr(
    ...
    fn test_create_string_from_char_ptr() {
        ...
        let basename = ffi::create_string_from_char_ptr(eid.basename as *mut c_char);
